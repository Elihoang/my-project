...
int dientichxay(int a[][100],int n){
    ...
    // tìm theo dau ma tran so voi cuoi ma tran  
    while(l<=r){
    	// chia doi ma tran  
        int mid=(l+r)/2;
        int flag=0;//danh dau 
        for(int i=0;i<=n-mid;i++){
            for(int j=0;j<=n-mid;j++){
                int dat=1;
                //cho a2 vong for chay duyet mang :phan tu khac 1 trong ma tran thì cho biên danh dau thanh 0 và ket thuc; 
        ...
        //bien dôi gai tri bien luu flag 
        //flag =1 con tro legft va right công len 
        if (flag==1) 
		l=mid+1;
        else 
		r=mid-1;
